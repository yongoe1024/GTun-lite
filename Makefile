.PHONY: build build-all build-linux build-darwin build-darwin-arm64 build-windows verify-manifest manifest test vet check schema clean clean-bin
...
build-linux: ## 构建 Linux amd64 程序包到 bin/linux/（先清空该平台目录）
	rm -rf bin/linux
...
build-darwin: ## 构建 macOS amd64 程序包到 bin/darwin/（先清空该平台目录）
	rm -rf bin/darwin
...
build-darwin-arm64: ## 构建 macOS arm64 程序包到 bin/darwin-arm64/（Apple Silicon 原生，先清空该平台目录）
...
build-windows: verify-manifest ## 构建 Windows amd64 程序包到 bin/windows/（先清空该平台目录；wintun.dll 必须与 exe 同目录）
	rm -rf bin/windows
...
build-all: clean-bin build-linux build-darwin build-darwin-arm64 build-windows ## 构建全部平台程序包（先整体清空 bin/）
