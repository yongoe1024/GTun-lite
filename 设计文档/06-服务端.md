...
- `submit`：**返回值契约 err 非 nil ⇔ 命令未生效**。排队阶段响应 ctx 取消与停机（此时命令尚未入队，丢弃无副作用）；入队成功后由 owner 停机排干保证执行（`commands` 无缓冲，owner 退出后新入队必被拒），等待段只等完成信号；
...
| 方法 | 语义 |
|---|---|
| `Register` | 审批分流（库里有 → upsert 刷新；新设备 → 只记 pendingDevices 不落库）→ 容量检查（只计已批准在线，正式名额不被待审批挤占；待审批走独立上限 128）→ 顶替旧会话（发 duplicate_login）→ `device_registered` + 首份 `network_config`（顺序即线上顺序） |
