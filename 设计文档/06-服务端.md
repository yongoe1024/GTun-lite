...
服务端是**会合/控制服务器**，不是数据面转发器：隧道数据在两个客户端之间 P2P 直传，服务器既不查表也不转发。服务端只下发意图（CONNECT/DISCONNECT/QUERY）并登记客户端上报的事实（权责划分见 [01-系统架构总览.md](01-系统架构总览.md) §6）。
...
`main` 只调 `os.Exit(run())`（os.Exit 不执行 defer，生命周期全在 run 内）。`run` 按序：
...
| 方法 | 语义 |
|---|---|
| `Register` | 容量检查（`server_full`）→ UpsertDevice → 顶替旧会话（发 duplicate_login）→ `device_registered` + 首份 `network_config`（顺序即线上顺序） |
...
2. **无认证**：协议没有鉴权 token——设备身份就是客户端自持的 UUID device_id，注册即 upsert。安全边界是网络层（管理面回环 + 控制面部署约定）。
...
每会话**唯一网络写者**保证下行顺序：从 `send` chan 取 → JSON marshal + LF → `SetWriteDeadline(write_timeout)` → 写出；`closeAfter` 消息写完即 `end()`。JSON Lines 分帧规则见 [02-公共协议层.md](02-公共协议层.md) §5。
...
链路状态是 hub owner 的纯内存状态，从不进 SQLite。服务器重启必然全部丢失——这是设计接受的行为（重建 = 客户端重连全量上报，论证见 [01-系统架构总览.md](01-系统架构总览.md) §6.3 与 [07-存储与管理面.md](07-存储与管理面.md)）。
...
- **gin → 标准库 net/http**：管理面全部是 JSON 序列化，不需要模板渲染/文件上传/复杂路由。Go 1.22+ ServeMux 的 `GET /path/{id}` 模式路由已够用；删除 gin 及其 15+ 间接依赖，供应链风险降低。gin.H → 结构体响应、`c.Param` → `r.PathValue`。
- **队列**：原版全局 10MB 字节级预算（queue_budget.go，三种语义）→ 固定长度 channel：控制面 `session.send`（满则阻塞 + 卡死超时关会话）、数据面队列在客户端（满则丢新包）。单机 8 设备内存压力极小，全局预算的协调成本大于收益。
