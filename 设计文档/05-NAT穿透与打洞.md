...
阶段3  打洞     按两侧画像组合选策略（§3），预算 = stable 5s / variable 15s
                期间完成三步握手（§4）
...
单端口探测被滤（如防火墙黑洞）意味着画像不完整 → 如实 PROBE_TIMEOUT（设计行为：单端口缺失 = 诚实失败，真机验证见 [10-真机验收记录.md](10-真机验收记录.md)）。
...
- **建成后不停发**——信标要一直播到打洞预算耗尽。原因：对端（stable 侧）可能尚未建成，它的握手材料就是这些信标；本侧先建成就停发，会让对端在预算内零握手来源（2026-08-27 公网真机教训，见 §4.3）；
...
- 分流：前 4 字节 == `"GTUN"` → 数据帧走 `deliverFrame` 四级验证（见 [04-客户端.md](04-客户端.md) §7.2）；否则按 P2P 控制包解析，**来源 IP 必须等于对端画像的公网 IP**（第三方伪造控制包直接丢弃）；
...
单向黑洞（一侧收不到任何隧道流量）场景：仅静默侧在 60s 超时并上报失活——对端因持续收到静默侧的**出站** PING 而保持存活（黑洞只挡静默侧的入站方向）。链路靠这一份单侧上报收敛，正是「任一侧报失败即失败」的用武之地。真机验证见 [10-真机验收记录.md](10-真机验收记录.md) D4。
...
- 打洞失败原因（Reason）随失败上报给服务器记录展示；服务器重启跨越打洞窗口时该原因丢失，接受不补偿（见 [01-系统架构总览.md](01-系统架构总览.md) §6.3）。
