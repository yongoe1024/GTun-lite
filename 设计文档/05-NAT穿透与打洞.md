...
|---|---|---|
| stable | `directPunch`：100ms 间隔直连对端稳定端点（5s 预算；成功仅需 2~3 个 RTT） | `rangeScan`：三级候选扫描（15s 预算） |
...
**真机实测结论**：现网运营商 NAT 多为随机分配端口，旧扫描不中；链路实际由「helper 信标 + 反向握手」建成，扫描是备援——二级/三级正是为「信标不可达（stable 侧过滤端口相关）」的环境补的确定性覆盖。

> **开发期观察设施（发布前删除）**：`scanlog.go` 把每发候选、入站信标的唯一源端口、命中溯源（哪一级、第几次猜中）写到工作目录 `gtun-scan.log`（写死路径、不走配置、失败静默丢弃），用于观察对端 NAT 端口分配规律。删除时连同 worker.go 中的调用点整体移除。
