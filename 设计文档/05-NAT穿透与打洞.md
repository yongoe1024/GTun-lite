...
> 窗口设计的依据：helper 映射在画像之后创建，NAT 分配的映射端口大概率排在观测端口**上方**；宽度随档位扩展（对称 ±48 窗口只够到第 48 个 helper，故只向上）。

**真机实测结论**：现网运营商 NAT 多为随机分配端口，扫描不中；链路实际由「helper 信标 + 反向握手」建成，Range 是备援。若未来遇到信标不可达的环境，回补步长外推（Profile 级预测）是首选扩展点。原版的五级端口预测（Profile/Bank/Range/Sweep/Birthday）因此砍成两级——Sweep/Birthday 覆盖率低，Profile/Bank 依赖复杂步长分析。
