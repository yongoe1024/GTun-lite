...
新设备注册不落库：hub 收到 device_register 后查 `HasDevice`——库里没有的设备只记入内存待审批表（name/platform 来自注册报文，随会话建立写入、随会话断开删除），控制会话照常建立、收空配置（不能入网=不能做任何操作）。管理页「同意注册」（POST approve）把条目落库为正式设备，此后重连走 upsert 刷新。被删除的设备重连会回到待审批列表，重新走审批。待审批设备的在线状态是实时的（存在即在线），不持久化。
