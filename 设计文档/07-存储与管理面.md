...
| links 视图 | 状态 chip（CONNECTING 黄 / CONNECTED 绿）、失败原因中文映射、token、设备名下虚拟 IP、更新时间（绝对 + 相对「x 分钟前」）、viewbar 全局开关「失败自动重试」（开启后每 5s 遍历链路表对断开的重发建链；手动断链的也会被扫回，断链前先关）、每行「刷新状态」（任一方在线即可用：对双方各发 QUERY，离线侧 409 自动跳过并提示，随后 1.5s 再刷）与建链/断链（双方在线才可用；CONNECTED 时建链禁用提示「重建请先断链」） | GET links / POST connect·disconnect·auto-retry / POST query ×2 |
