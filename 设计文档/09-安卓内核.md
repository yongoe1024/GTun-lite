...
4. 内核置非阻塞后 `os.NewFile(fd)` 包成 tun.Device,数据面读写循环启动
   (宿主交付的是阻塞 fd;非阻塞才进 runtime poller,Close 才能唤醒读循环,
   与 03 篇桌面平台同款约束)。
...
| # | 约束 | 依据 |
|---|---|---|
| 1 | **同步请求-应答**:RequestTun 直接返回 fd,调用返回即无状态残留 | 会话状态全部活在 Open 调用栈;跨会话暂存要求生命周期对齐约定,易错 |
| 2 | **授权在会话启动前解决**:宿主先 prepare(弹框)再 Start | establish 是纯系统调用不碰 UI,是同步等待无阻塞风险的前提 |
| 3 | **fd 一次性**:每次开栈重新 establish,不缓存 | fd 随 VPN 接口生灭,缓存必失效 |
| 4 | **所有权显式移交**:Java detachFd ↔ Go Close | 同一 fd 双重关闭会抽走对方正在用的把手 |
| 5 | **回调注册先于首次开栈**(gtunlib.run 装配时 SetTunRequester) | 未注册时 Open 即「requester not set」失败 |
| 6 | **失败原因原路上报**:Java 异常消息 → error → Reason → 服务端 | 页面与服务端须直读失败原因,不能只看到失败 |
