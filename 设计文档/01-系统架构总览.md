...
> 本章是全文档集的主线：先给项目定位与三大平面，再给端到端建链时序，然后是贯穿全部模块的设计哲学与权责划分，最后是相对前代实现（原版 GTun）的取舍记录。后续各模块文档（02–07）只展开各自层面的细节，不再复述本章结论。
...
不显示「未知」：状态确实是已知的（客户端最后一次亲口说的），隧道不经过服务器，离线期间 CONNECTED 比「未知」更接近现实；必须带时间戳，否则缓存值会被误读成当前状态；在线性单独一段，因为它是唯一实时可信的信息。
...
原版指已废弃的前代 GTun 实现，不在本仓库、不再维护；对比只作为砍除决策的论证记录保留，防止机制被回头加回去。一张表同时回答「原来怎么做、现在怎么做、为什么不需要原来的」：
